...
import { createPrediction, getPrediction } from '../utills/replicate';
...
export default function Home() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const initialPrediction = await createPrediction({ image: input });
      let finalPrediction = initialPrediction;

      while (finalPrediction.status !== 'succeeded' && finalPrediction.status !== 'failed') {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        finalPrediction = await getPrediction(initialPrediction.id);
      }
      ...
      {prediction && (
        <div className="mt-4">
          {prediction.status === 'succeeded' ? (
            <div>
              <h2 className="text-xl font-bold mb-2">Enhanced Image</h2>
              <img src={prediction.output} alt="Enhanced" className="border" />
            </div>
          ) : (
            <p className="text-red-500">Failed to enhance image</p>
          )}
        </div>
      )}
    </main>
